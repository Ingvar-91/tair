...
@section('title', 'Отзывы о магазине')

@section('content')

<nav class="box box-control">
    {!! Breadcrumbs::render('reviews.shop', $shop) !!}
</nav>
    ...
    <div class="row custom-row">
        ...
        <div class="col-sm-4 m-l-sm">
            <div class="box m-r p-l p-r p-b">
                <div class="h3">
                    <a href="{{Route('shop', ['shop_id' => $shop->id])}}">{{$shop->title}}</a>
                </div>
                <div class="img">
                    @if(Helper::getImg($shop->images, 'shops', 'large'))
                        <img src="{{Helper::getImg($shop->images, 'shops', 'large')}}" class="img-responsive w-100" alt=""/>
                    @endif
                </div>
            </div>
        </div>
